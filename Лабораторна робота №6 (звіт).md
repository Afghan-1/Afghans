...
Виконав Нестропа Максим Романович 
 Ось таблиця з командами та їх описом:
Команда	Призначення та функціональність
mkdir mybackups	Створює новий каталог mybackups у домашній директорії користувача.
tar -cvf mybackups/udev.tar /etc/udev	Створює архів udev.tar у каталозі mybackups, що містить /etc/udev. Прапори: -c (створити), -v (вивести процес), -f (вказати файл).
...
tar -xvf mybackups/udev.tar	Витягує вміст архіву udev.tar. Прапори: -x (витягти), -v (вивести процес), -f (вказати файл).
tar -czvf archive.tar.gz directory/	Створює стиснутий gzip-архів archive.tar.gz для вказаного каталогу. Прапор -z вмикає gzip-стис
